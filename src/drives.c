	...
	while (fgets(mount_line, sizeof(mount_line), fp)) {
		...
		*type_pos = '\0';
		...
		snprintf(cmd, sizeof(cmd),
			 "df -B1 \"%s\" 2>/dev/null | tail -n1", mountpoint);

		FILE *dfp = popen(cmd, "r");
		if (!dfp) continue;

		if (fgets(df_result, sizeof(df_result), dfp)) {
			char fs_name[128], mount_str[128], usedp_str[8],
			    avail_str[64];
			long long total_bytes, used_bytes, avail_bytes;
			float used_percent;

			int parsed =
			    sscanf(df_result, "%s %lld %lld %lld %s %s",
				   fs_name, &total_bytes, &used_bytes,
				   &avail_bytes, usedp_str, mount_str);

			if (parsed == 6) {
				sscanf(usedp_str, "%f%%", &used_percent);

				int total_blocks = 30;
				int filled = (int)((used_percent / 100.0) *
						   total_blocks);
				...
				char used_total[32];
				snprintf(used_total, sizeof(used_total),
					 "%6.2f/%-6.2fGB",
					 used_bytes / (1024.0 * 1024 * 1024),
					 total_bytes / (1024.0 * 1024 * 1024));

				printf("%-30s %-15s [%-30s] %3.0f%%\n",
				       mountpoint, used_total, blocks,
				       used_percent);
			}
		}
