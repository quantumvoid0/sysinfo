...
void proc_box(WINDOW *win, int scroll, int selected_line) {
    ...
    FILE *fp = popen("ps -eo pid,comm,user,%cpu,%mem,tty,start_time --sort=-%cpu", "r");
    if (!fp) {
...
int main() {
    ...
    while (1) {
        ...
        WINDOW *graph_win = newwin(bottom_h, graph_w_part, graph_start, 0);
        WINDOW *proc_win  = newwin(bottom_h, proc_w_part, graph_start, graph_w_part);

        if (!graph || graph_w != graph_w_part) {
            if (graph) free(graph);
            graph_w = graph_w_part; graph_h = bottom_h;
        ...
        const char *sel_text =
            (selected==SEL_CPU)  ? "CPU FREQ" :
            (selected==SEL_RAM)  ? "RAM" :
            (selected==SEL_SWAP) ? "SWAP" :
                                   "PROCESSES";

        snprintf(status_text, sizeof(status_text),
            "Selected: %s  |  LEFT/RIGHT switch  UP/DOWN scroll  Q quit",
            sel_text);
        ...
        if (ch == 'q') break;
            ...
            if (selected == SEL_CPU) selected = SEL_PROC;
            else selected = (selected-1);
        }
        else if (ch == KEY_RIGHT) {
            if (selected == SEL_PROC) selected = SEL_CPU;
            else selected = (selected+1);
        }
